...
Credits: Lecture and literate Haskell file borrowed from Ranjit Jhala, UCSD CSE 230 
...
Recall the `Shape` type from a few lectures back (here we'll just 
use the definitions in the SOE library.)
...
> demo9 = animateB "9" $ a1 `over` a2 
>   where a1 = reg red $ tx (sin time, cos time) ballB
>         a2 = timeTx (0.5 * time) a1 

10. or go backwards in time (rewind) 

> demo0 = animateB "0" $ a1 `over` a2 
>   where a1 = reg red $ tx (sin time, cos time) ballB
>         a2 = timeTx (-1 * time) a1 
...
> anim6 = animateB "Pendulum" $ f pentaB
>   where f = reg flash'' 
